...
TwoTerm_fit <- function(time, moisture) {
  tryResult <- tryCatch(nls(moisture ~ a * exp(-k1 * time) + b * exp(-k2 * time), start = list(a = 0.3, k1 = 0.5, b = 0.5, k2 = 0.1)),
                        error=function(e) {
                          print(e)
                          print(strsplit(as.character(e), ":")[[1]][2])
                        }
  ...
  if (class(tryResult) == "nls") {  
    model <- nls(moisture ~ a * exp(-k1 * time) + b * exp(-k2 * time), start = list(a = 0.3, k1 = 0.5, b = 0.5, k2 = 0.1))
    Tt <- predict(model)
  ...
  if (file.info(infile)$size > 0) {
    ...
    if (MBData[2,] == "OHAUS MB45 SN   1122272694") {
      ...
      if (length(initialWeight) == 2 * nSamples) {
        ...
        cat(infile, ", Data mismatch error.\n", file = logfName, append = TRUE)
      }
    } else {
      
      # The file contains no data
      
      cat(infile, ", The file does not contain MB45 data.\n", file = logfName, append = TRUE)
    }
  } else {
    
    # The file has size less or equal to zero
    
    cat(infile, ", The file has size less than or equal to zero.\n", file = logfName, append = TRUE)
  }
}
