          ...
          if (endData[i] - beginData[i] > 1) {
            ...
            if (nModels > 0) {
              ...
              for (p in 1:nModels) {
                ...
                modelFormula <- summary(get(corrDF$modelAbbr[p])[[1]])$formula
                modelEquation <- paste("moisture ~", modelFormula[3])
                
                # The model parameters generated by summary() are:  Estimate, Std. Error, t value, Pr(>|t|)
                
                modelParams <- summary(get(corrDF$modelAbbr[p])[[1]])$parameters
