...
nls(moisture ~ exp(-(k * t) ^ n), start = list(k = 0.5, n = 0.5))
...
MP_moisture <- exp(-(1.185 * t) ^ 1.278)

cor(MP_moisture, moisture)

plotData <- data.frame(t, moisture, MP_moisture)

head(plotData)

ggplot(plotData, aes(x = t, y = moisture)) +
  geom_point(size = 2) +
  geom_line(data=plotData, aes(x = t, y = MP_moisture), color = 'darkgreen', size = 1.5)
...
head(rawData[, c("dectime", "MR")])
  ...
  tryResult <- tryCatch(nls(moisture ~ exp(-(k * (t / 0.43 ^ 2) ^ n)),
                          #  start = list(k = 0.5, L = 0.1, n = 0.5)),
                            start = list(k = 0.5, n = 0.5)),
                        error=function(e) {
                          print(e)
                          print(strsplit(as.character(e), ":")[[1]][2])
                        }
...
MPII_moisture <- exp(-(k * (t / L ^ 2) ^ n))

cor(MPII_moisture, moisture)

plotData <- data.frame(t, moisture, MPII_moisture)
...
ggplot(plotData, aes(x = t, y = moisture)) +
  geom_point(size = 2) +
  geom_line(data=plotData, aes(x = t, y = MPII_moisture), color = 'orange', size = 1.5)
```
...
nls(moisture ~ exp(-(k * t) ^ n), start = list(k = 1.185, n = 1.278))

```

### One possible explanation is that there are "too many" possible solutions given the 3 fitting parmeters.  If we only use 2 parameters:
...
nls(moisture ~ exp(-(k * (t / 0.1 ^ 2) ^ n)), start = list(k = 0.5, n = 0.5))

```

### Self starting?

```{r MPII self starting}

# k = 0.5
# L = 0.1
# n = 0.5

partial <- seq(1, length(rawData$MR), 5)

moisture <- rawData$MR[partial]
t <- rawData$dectime[partial]

fixed_k <- nls(moisture ~ exp(-(0.5 * (t / L ^ 2) ^ n)), start = c(L = 0.1, n = 0.5))
fixed_L <- nls(moisture ~ exp(-(k * (t / 0.1 ^ 2) ^ n)), start = c(k = 0.5, n = 0.5))
fixed_n <- nls(moisture ~ exp(-(k * (t / L ^ 2) ^ 0.5)), start = c(k = 0.5, L = 0.1))

nls(moisture ~ exp(-(k * (t / L ^ 2) ^ n)), start = c(k = -0.1, L = 0.7, n = 1.28), warnOnly = TRUE)
